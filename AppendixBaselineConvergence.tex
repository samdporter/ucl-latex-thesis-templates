...
\begin{figure}[htbp]
    ...
    \caption{Relative PET iterate velocity, $\lVert x^{(k)}-x^{(k-1)}\rVert/\lVert x^{(k)}\rVert$, and relaxed step size during the long-run reference reconstruction. The final velocity is $7.285\times10^{-8}$ per epoch while the step size remains $0.2174$.}
...
\begin{figure}[htbp]
    ...
    \caption{Final PET and SPECT iterates of the long-run reference reconstruction, shown on a matched slice with independent colour bars. This pair is the reference $x^\star$ against which the NRMSE curves of Chapter~\ref{chapter:optimisation} are measured; it is the long-run baseline rather than one of the candidate preconditioner settings compared there.}
    \label{fig:app_baseline_reference_pair}
\end{figure}
