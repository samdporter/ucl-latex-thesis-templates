...
Let $J_k=\mathcal O(\bm z^{(k)})$ be the recorded objective after epoch $k$, and define
\begin{equation}
    J^\star=\min_{0\leq k\leq 1000}J_k.
    \label{eq:app_baseline_best_objective}
\end{equation}
Here $J^\star$ is the best objective observed during this run. It is not a proven global minimum, and the reference image should likewise be understood as a long-run numerical reference rather than ground truth.
...
\begin{figure}[htbp]
    ...
    \end{subfigure}
    \caption{Long-run objective diagnostics. The solid curve is $J_k-J^\star$ and the dashed curve is the relaxed step size. The linear view shows that most of the observed objective decrease occurs early, while the logarithmic view resolves the residual decrease over the complete 1000-epoch run.}
