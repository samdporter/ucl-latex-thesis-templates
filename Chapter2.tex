...
To better preserve edges, \gls{tv} \cite{rudin_nonlinear_1992} replaces the squared penalty with an $\ell_1$-type penalty on local differences. A neighbourhood-difference form that makes the connection to $\mathcal{R}_{\mathrm{Quad}}$ explicit is
\begin{equation}
...
\end{equation}
with $\gamma$ controlling the amount of edge preservation. Note that if $(x_j+x_k)\gg \gamma|x_j-x_k|$ then $\mathcal R_{\mathrm{RDP}}$ behaves like a relative quadratic prior,
...
\end{equation}
which penalises predominantly the gradient component orthogonal to the anatomical level-set direction, thereby encouraging edges in $x$ to align with edges in $u$. In regions where $u$ is locally flat (so $\xi_j \approx 0$), $\Pi_j \approx I$ and \gls{dtv} reduces to standard isotropic \gls{tv}.
...
When the native modalities are acquired on different grids or coordinate frames, we introduce spatial transforms $W_m$ (registration + resampling operators) and define the co-registered images on a shared grid, with $W_m: \mathbb{R}_+^{J_m} \rightarrow \mathbb{R}_+^N$:
\begin{equation}
...
\begin{equation}
    ...
    \right\},
\end{equation}
where $\bm{x}\coloneqq(x_1,\dots,x_M)$, $\bm{z}=\bm{W}\bm{x}$ with $\bm{W}\coloneqq\mathrm{diag}(W_1,\dots,W_M)$, and $\mathcal{R}(\bm{z})$ promotes cross-modality consistency (e.g.\ co-located edges or shared support) in the co-registered domain.
...
\begin{align}
    ...
    \operatorname{diag}\!\left(\frac{x^{(k)}}{A^{\mathsf T}\mathbf{1}_I}\right)
        D \mathcal{L}(x^{(k)}).
\end{align}
where $\mathbf{1}_J\in\mathbb{R}^{J}$ is an all-ones vector. This form highlights that the \glsentryshort{mlem} update can be interpreted as a preconditioned gradient descent algorithm with preconditioner $P_{EM}^{(k)}=\operatorname{diag}\!\left(\frac{x^{(k)}}{A^{\mathsf T}\mathbf{1}_I}\right)$, i.e.
\begin{align}
...
\end{equation}
If multiple subsets are used per iteration, $D_x \mathcal{L}_{s}$ is replaced by an average (or sum) over the chosen subset indices. The most common algorithm found in clinical systems is the \gls{osem} algorithm. For subset $s$ at iteration $k$, a standard \glsentryshort{osem} update can be written as
\begin{equation}
x^{(k+1)}
=
...
\glsentryshort{osem} typically shows a marked improvement in early-stage convergence by providing a cheaper, higher-variance approximation of the full-data \glsentryshort{em} update. However, because the subset gradient is only an approximation to the full gradient, \glsentryshort{osem} is not, in general, a convergent algorithm for the maximum-likelihood problem: as the iterates approach a solution, the stochasticity introduced by cycling through subsets can dominate, leading to limit-cycle behaviour in which successive subset updates fail to settle to a fixed point (Figure~\ref{fig:osem_limit_cycle}).
...
We seek a fast and convergent algorithm for \gls{map} reconstruction. A widely used approach is the (modified) \gls{bsrem} framework \cite{ahn_globally_2002}, which combines \glsentryshort{em}-type preconditioning and subset acceleration with a relaxation (diminishing step-size) to recover convergence while incorporating an explicit penalty. In a scaled-gradient form, a generic \glsentryshort{bsrem}-type update can be written as
\begin{equation}
    x^{(k+1)}
    =
...
A central theme in modern \gls{pet}/\gls{spect} practice is the availability of multiple co-registered modalities. While sequential (one-way) guidance can be effective, it is inherently limited by asymmetric information flow and by the risk of propagating errors or mismatches from the guidance image into the emission reconstruction. This motivates synergistic reconstruction, in which modalities are coupled through joint priors. In this setting, Jacobian-based penalties such as \glsentryshort{jtv} and \glsentryshort{tnv} provide a natural mechanism to promote co-located, directionally consistent edges across modalities while allowing modality-specific contrast.
...
\begin{enumerate}
    \item \textbf{Synergistic, anatomy-informed coupling:} existing Jacobian-based priors (e.g.\ \gls{tnv}) are typically formulated to promote cross-modality edge consistency, but are not routinely combined with explicit directional guidance derived from high-resolution \glsentryshort{ct} edges in a way that remains robust in low-count regimes.
