...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption{Projection-domain behaviour of the preparatory additive-term refinement for a measured NEMA phantom. Coloured curves are profiles of the estimated non-primary term $b^{(k)}$; every displayed iteration belongs to the preparatory reconstruct--simulate loop. Profiles are summed over views at an axial position through the spheres; the horizontal coordinate is the tangential detector bin and the vertical coordinate is counts. The measured total-emission profile and learned scatter initialisation are scale and shape references, not ground truth for $b^{(k)}$. Full replacement
