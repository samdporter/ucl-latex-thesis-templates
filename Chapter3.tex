...
\begin{equation}
    ...
    \label{eq:brem}
\end{equation}
The Gaussian form is motivated by the external-photon displacement distributions of Rault \emph{et al.}, while its effective width was estimated from the water-equivalent Monte Carlo response study described below \cite{raultFastSimulationYttrium902010}. A single $6.9\,\mathrm{mm}$-\gls{fwhm} kernel ($\sigma_{\mathrm{eff}}\approx2.93\,\mathrm{mm}$) was then used throughout the reconstructions. This is not a microscopic displacement \glsentryshort{pdf}: by applying one Gaussian to all source activity, it approximates the combined internal/external response with one fitted width.
