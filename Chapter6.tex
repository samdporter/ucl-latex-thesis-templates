...
\SA{Just checking : have you defined how the non-negativity prior works ? As an additive prior it should be some kind of barrier function e.g. $\iota_z = \infty ~\textnormal{if}~ z < o, ~\textnormal{else}~ z $}
...
\end{equation}
be the weighted directional Jacobian from Chapter~\ref{chapter:wdtnv} \SA{Give an equation referenc and/or page reference}. The active code forms the modality-space \glsentryshort{mm} weight
\begin{equation}
    C_n
    =
...
\end{equation}
This matrix is the same $2\times2$ inverse square-root used in the gradient calculation\SA{Refer to where that was done}, now interpreted as the local quadratic-surrogate weight.
...
\end{equation}
where $B_r=\mathrm{diag}(\beta_{1,r},\beta_{2,r})$ and $\circ$ denotes the Hadamard product.
...
Let $P_{\mathrm D}=\widehat H_{\mathrm D}^{-1}$ denote the data preconditioner and $P_{\mathcal R}=\widehat H_{\mathcal R}^{-1}$ the preconditioner obtained from one of the prior-curvature models above. For block methods, $\widehat H_{\mathrm D}$ is the $2\times2$ diagonal matrix containing the modality-specific data curvatures at each voxel. The principal comparison combines the operands through the inverse-sum, or parallel-sum, rule
\begin{equation}
    P_{\parallel}
    =
...
The inverse sum is one member of a broader matrix Lehmer family. For voxel-wise symmetric positive-definite preconditioners $B=P_{\mathcal R}$ and $D=P_{\mathrm D}$, define
\begin{equation}
    C=D^{-1/2}BD^{-1/2},
    \qquad
    L_q(B,D)=D^{1/2}f_q(C)D^{1/2},
    \qquad
    f_q(t)=\frac{t^q+1}{t^{q-1}+1},
    \label{eq:matrix_lehmer_combination}
\end{equation}
where $f_q(C)$ is evaluated spectrally. The implemented preconditioner is $\tfrac12L_q(B,D)$. At $q=0$ this reduces exactly to the parallel sum,
\begin{equation}
    \tfrac12L_0(P_{\mathcal R},P_{\mathrm D})
    =
    \left(P_{\mathrm D}^{-1}+P_{\mathcal R}^{-1}\right)^{-1}.
    \label{eq:lehmer_parallel_equivalence}
\end{equation}
The legends in Figure~\ref{fig:optim_lehmer_parallel} use $p$ for the same order denoted by $q$ in the text.
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}{0.99\linewidth}
        ...
        \caption{SPECT represented on the PET grid.}
    \end{subfigure}
    \caption{Parallel-sum and Lehmer combinations for the Lewis--Sendov block-diagonal prior model. This experiment changes only the combination rule; lines and shading show the five-run mean and pointwise minimum--maximum over 20 matched data passes. The plot legend denotes the Lehmer order by $p$, whereas the text uses $q$.}
    \label{fig:optim_lehmer_parallel}
\end{figure}

For the Lewis--Sendov block model, $q=0.01$ is marginally better than the inverse-sum result in these curves, but the difference is small. This comparison does not establish that the same order is preferable for the MM-derived preconditioners. Chapter~\ref{chapter:paper} had already frozen the Lewis--Sendov block-diagonal model with $q=0.01$ before the broader benchmark in this chapter. Retaining it therefore provides protocol continuity, not a post-hoc claim that Lewis--Sendov or $q=0.01$ is universally optimal.
...
The Lewis--Sendov Lehmer experiment found only a small advantage for $q=0.01$ over the parallel sum. Chapter~\ref{chapter:paper} retains the previously frozen Lewis--Sendov block-diagonal/$q=0.01$ configuration for protocol continuity. Its use in that chapter should not be interpreted as selection of the best method after the wider comparison reported here.
