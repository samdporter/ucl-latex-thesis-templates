...
r=\min\{d,M\},
\end{equation}
is orthogonally invariant and depends on $Y$ only through its singular values $\sigma(Y)=(\sigma_1(Y),\dots,\sigma_r(Y))$. The gradient of such a function is given by the standard formula for orthogonally invariant matrix functions \cite{lewisConvexAnalysisUnitarily1995,lewisDerivativesSpectralFunctions1996}. That is, 
\[
\nabla\phi(Y)
=
U\operatorname{Diag}\!\left(
g'(\sigma_1(Y)),\ldots,g'(\sigma_r(Y))
\right)V^\top.
\]
Since \(g'(0)=0\), this formula also holds when \(Y\) is rank deficient.
