...
For nuclear-norm vectorial total variation, the unsmoothed singular-value map is not differentiable at zero singular values. We therefore apply the Charbonnier function to the singular values:
...
The nuclear norm is orthogonally invariant. That is, for any orthogonal $U\in\mathbb{R}^{d\times d}$ and $V\in\mathbb{R}^{M\times M}$,
\begin{equation}
\|Y\|_* \;=\; \|U Y V^\top\|_*.
\end{equation}
...
Smoothness of $\phi$ does not follow from smoothness of $g$ on $[0,\infty)$ alone. The Lewis--Sendov theory of spectral functions requires $f$ to be \emph{absolutely symmetric}, that is invariant under both permutations and sign changes of its arguments, and requires differentiability of $f$ as a function on $\mathbb{R}^r$ rather than on the nonnegative orthant \cite{lewisConvexAnalysisUnitarily1995,lewisDerivativesSpectralFunctions1996}. The distinction is not cosmetic: the unsmoothed choice $g(s)=s$ is also $C^\infty$ on $[0,\infty)$, yet the nuclear norm is not differentiable at rank-deficient points. What licenses differentiability here is that for $\epsilon>0$ the Charbonnier function
\begin{equation}
    g(s)=\sqrt{s^2+\epsilon^2}-\epsilon
\end{equation}
extends to an \emph{even} function that is $C^\infty$ on all of $\mathbb{R}$, and in particular across $s=0$. Consequently $f(s)=\sum_{\ell}g(s_\ell)$ is absolutely symmetric and $C^\infty$ on $\mathbb{R}^r$, so $\phi=f\circ\sigma$ is differentiable everywhere, including at rank-deficient $Y$, with gradient given by the standard formula for orthogonally invariant matrix functions \cite{lewisConvexAnalysisUnitarily1995,lewisDerivativesSpectralFunctions1996}. Singular-value smoothing is therefore the spectral counterpart of the corner rounding used for \gls{tv}, where $|z|$ is replaced by $\sqrt{z^2+\epsilon^2}$ and the resulting penalty is recognised as ordinary edge-preserving regularisation with a hyperbola potential function. \TODO{Add the Zenodo-managed citations for Charbonnier et al.\ (1997) on deterministic edge-preserving regularisation and for Fessler, \emph{Image Reconstruction: Algorithms and Analysis}, Section 2.4.5, where corner-rounded TV is identified with the hyperbola potential.}
...
\end{equation}
Combined with the convexity of the Poisson negative log-likelihood and of the non-negativity indicator, this makes the \gls{map} objective of Chapter~\ref{chapter:optimisation} convex, which is what allows the long-run reference reconstruction used there to be interpreted as approaching a global minimiser rather than an arbitrary stationary point.
...
It is worth being explicit that voxel-wise weighting is a genuine change of prior rather than a reparametrisation. Because the nuclear norm is positively homogeneous, a spatially constant weight $\rho_{m,n}\equiv\rho_m$ would make $\mathcal{R}_{\mathrm{w\text{-}dTNV}}$ identical to $d$TNV evaluated on the rescaled images $\rho_m z_m$. This equivalence fails as soon as the weights vary spatially, since $\rho_{m,n}(D z_m)_n \neq \bigl(D(\rho_m z_m)\bigr)_n$ in general. The weighting therefore alters the estimator and not merely the scaling of the variables, a point returned to below.
...
The motivation given above, that the coupling should not be dominated by the modality with larger gradient magnitudes, is phrased in the language of conditioning, but $B_n$ changes the estimator and not only the geometry seen by the optimiser. This distinction matters because relative weighting between coupled channels controls the spatial resolution and noise properties of the individual reconstructions, and controlling those properties is known to be difficult for coupled multi-channel regularisers precisely when the channel data statistics differ. \gls{pet} and bremsstrahlung \gls{spect} of the same $^{90}$Y distribution differ in count level, sensitivity and intrinsic resolution simultaneously, which is the least favourable case for that concern. The weights are therefore best understood as encoding relative edge credibility between modalities, and the operating point selected in Chapter~\ref{chapter:paper} is chosen on a task-based trade-off rather than justified by convergence behaviour. A local impulse-response analysis of the resolution actually delivered to each modality at a given $B_n$ was not performed and remains a natural extension. \TODO{Add the Zenodo-managed citation for Fessler, \emph{Image Reconstruction: Algorithms and Analysis}, Section 2.8.2, where the difficulty of controlling per-channel spatial resolution under coupled multi-channel regularisation is raised.}
...
The smoothed spectral weight $\Omegaeps{\tilde{\mathcal{G}}_n}$ of~\eqref{eq:spectral_weight}, appearing in~\eqref{eq:2x2_gram}, is not merely a computational convenience. Chapter~\ref{chapter:optimisation} shows that it is simultaneously the weight of a quadratic majoriser of the prior, making it the matrix-valued analogue of the optimal curvature of a scalar edge-preserving potential, and this is why the same quantity serves both the gradient and the preconditioner.
