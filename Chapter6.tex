...
The motivation given above, that the coupling should not be dominated by the modality with larger gradient magnitudes, is phrased in the language of conditioning, but $B_n$ changes the estimator and not only the geometry seen by the optimiser. This distinction matters because relative weighting between coupled channels controls the spatial resolution and noise properties of the individual reconstructions, and controlling those properties is known to be difficult for coupled multi-channel regularisers precisely when the channel data statistics differ. \gls{pet} and bremsstrahlung \gls{spect} of the same $^{90}$Y distribution differ in count level, sensitivity and intrinsic resolution simultaneously. The weights are therefore best understood as encoding relative edge credibility between modalities, and the operating point selected in Chapter~\ref{chapter:paper} is chosen on a task-based trade-off rather than justified by convergence behaviour. 
