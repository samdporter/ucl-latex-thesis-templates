...
\end{equation}
so that~\eqref{eq:phi_grad_inv_sqrt} reads $\nabla_{\mathcal{G}}\phi(\mathcal{G})=\mathcal{G}\,\Omegaeps{\mathcal{G}}$. Because $\epsilon>0$, $\Omegaeps{Y}$ is well defined and \gls{spd} for every $Y$, including rank-deficient ones. For $M=1$ it collapses to the scalar $(\|Y\|_2^2+\epsilon^2)^{-1/2}$, which is the quadratic-surrogate (IRLS) weight for the hyperbola potential; it is not its second derivative. Chapter~\ref{chapter:optimisation} uses this surrogate weight in its majorisation construction.
...
Throughout, $B_n$ is treated as a fixed operator that does not depend on $\bm{z}$. In the application of Chapter~\ref{chapter:paper} the weights are computed once, from early-stopped unregularised reconstructions, and then held fixed. Recomputing them from the current iterate would require a separate analysis because the resulting update is not the gradient of the stated fixed-weight objective. The weights also absorb the overall regularisation strength; there is no separate global scalar prior multiplier in~\eqref{eq:wdtnv_prior}, because the modality-specific regularisation weights $\beta_m$ enter through $\rho_{m,n}$ (Section~\ref{subsec:90y_global_weights}).
