...
\end{equation}
where $\mathcal{D}_i$ are subset data-fidelity terms, $\mathcal{R}_{\mathrm{w\text{-}dTNV}}$ is the prior from Chapter~\ref{chapter:wdtnv}, and $\iota_{\mathbb{R}_+}$ enforces non-negativity. 
...
Each $\mathcal{D}_i$ is a Poisson negative log-likelihood term composed with the relevant projector, image-space registration operator, and optional resolution model.
